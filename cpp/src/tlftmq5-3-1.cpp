...
{
    cout << name << " is doing coding work in " << specialty << "." << endl ;
}
...
int main() 
{
    ...
    for (auto& m : managers) 
    {
        outFile << "Manager," << m.getName() << "," << m.getID() << "," << m.getSalary() << "," << m.getTeamSize() << endl ;
    }

    for (auto& e : engineers) 
    {
        outFile << "Engineer," << e.getName() << "," << e.getID() << "," << e.getSalary() << "," << e.getSpecialty() << endl ;
    }
    ...
    cout << endl << "===== Work Info from File =====" << endl ;
